...
const Runners: React.FC<RunnersProps> = ({ isDarkMode, onAddRunner }) => {
    ...
    const fetchRunners = async () => {
        try {
            ...
            const response = await fetch('http://localhost:5000/api/runners/runners-get', {
    ...
    const handleToggleStatus = async (id: string, currentStatus: string) => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/runners/${id}/status`, {
                method: 'PATCH',
                headers: {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:5000/api/runners/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
