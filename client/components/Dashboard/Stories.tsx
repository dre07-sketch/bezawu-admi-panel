...
const Stories: React.FC<StoriesProps> = ({ isDarkMode, onAddStory }) => {
    ...
    const fetchStories = async () => {
        try {
            ...
            const response = await fetch('http://localhost:5000/api/stories/stories-get', {
    ...
    useEffect(() => {
        if (selectedStory) {
            const fetchStoryDetails = async () => {
                try {
                    const token = localStorage.getItem('token');
                    const response = await fetch(`http://localhost:5000/api/stories/${selectedStory.id}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
                        const data = await response.json();
            ...
            const fetchComments = async () => {
                ...
                try {
                    const token = localStorage.getItem('token');
                    const response = await fetch(`http://localhost:5000/api/stories/${selectedStory.id}/comments`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (response.ok) {
                        const data = await response.json();
    ...
    const handleToggleStatus = async (e: React.MouseEvent, id: string) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:5000/api/stories/${id}/toggle`, {
                method: 'PATCH',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
