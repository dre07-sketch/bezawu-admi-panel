...
const GlobalTermination: React.FC<GlobalTerminationProps> = ({ onRestore, onLogout }) => {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 overflow-hidden transition-all duration-1000 bg-[#050505] selection:bg-rose-500/30">
      ...
      <div className={`absolute w-[800px] h-[800px] rounded-full blur-[150px] animate-morph transition-colors duration-1000 ${glitch ? 'bg-rose-900 opacity-40 scale-110' : 'bg-rose-600 opacity-20'
        }`}></div>
      ...
      <div className={`relative z-10 max-w-2xl w-full text-center space-y-12 transition-transform duration-300 ${glitch ? 'translate-x-1 skew-x-1' : ''}`}>
        ...
        <div className="flex flex-col items-center gap-6">
          {onLogout && (
            <button
              onClick={onLogout}
              className="px-8 py-3 rounded-xl border border-rose-500/20 text-rose-500 font-black text-[10px] uppercase tracking-widest hover:bg-rose-500/10 transition-all active:scale-95"
            >
