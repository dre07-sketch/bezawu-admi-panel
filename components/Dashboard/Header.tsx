...
const Header: React.FC<HeaderProps> = ({ user, isBusy, onToggleBusy, isDarkMode, onToggleTheme }) => {
  return (
    <header className={`h-14 border-b px-8 flex items-center justify-between sticky top-0 z-10 backdrop-blur-md transition-colors duration-300 ${
      isDarkMode ? 'bg-[#0f1115]/80 border-slate-800' : 'bg-white/80 border-slate-200'
    }`}>
      <div className="flex items-center gap-6">
        <div>
          <h2 className={`text-base font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{user.branchName}</h2>
          <div className="flex items-center gap-2">
            <div className={`h-1.5 w-1.5 rounded-full ${isBusy ? 'bg-red-500' : 'bg-emerald-500'} animate-pulse`}></div>
            <span className={`text-[9px] font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
              {isBusy ? 'Busy Mode' : 'System Live'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className={`flex items-center border rounded-xl px-4 py-1.5 w-64 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all ${
          isDarkMode ? 'bg-[#121418] border-slate-800 focus-within:border-emerald-500/50' : 'bg-slate-50 border-slate-200 focus-within:border-emerald-500'
        }`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button className={`p-2 border rounded-xl transition-all relative ${
          ...
          <div className="flex items-center gap-3">
            ...
            <div className={`h-9 w-9 border rounded-xl flex items-center justify-center text-emerald-500 ${
